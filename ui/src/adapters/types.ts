...
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "@yantra/adapter-utils";

export interface StatefulStdoutParser {
  parseLine: (line: string, ts: string) => import("@yantra/adapter-utils").TranscriptEntry[];
  reset: () => void;
}
...
export interface TranscriptParserSource {
  parseStdoutLine: (line: string, ts: string) => import("@yantra/adapter-utils").TranscriptEntry[];
