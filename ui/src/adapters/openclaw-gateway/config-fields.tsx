...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="Yantra API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "yantraApiUrl",
                  String(config.yantraApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "yantraApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://yantra.example"
            />
          </Field>
          ...
          <Field label="Claimed API key path">
            <DraftInput
              ...
              immediate
              className={inputClass}
              placeholder="~/.openclaw/workspace/yantra-claimed-api-key.json"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "yantra"))}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. Yantra persists a device key during onboarding so pairing approvals
