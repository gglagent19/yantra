 ...
 *
 * Plugin bundles are built with `external: ["@yantra/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__yantraPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__yantraPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__yantraPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__yantraPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@yantra/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@yantra/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@yantra/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@yantra/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@yantra/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__yantraPluginBridge__) {
 ...
 * self-register via `window.yantraPlugins.registerReactComponent()`.
 ...
 * of `@yantra/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
