...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("yantra-worktree-enabled") !== "true") return null;

  const name = readMetaContent("yantra-worktree-name");
  const color = normalizeHexColor(readMetaContent("yantra-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("yantra-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
