...
/**
 * Filter .yantra.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterYantraYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://yantra.ing) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm yantraai company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [Yantra](https://yantra.ing) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [Yantra](https://yantra.ing) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .yantra.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .yantra.yaml
    const yamlPath = exportData.yantraExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterYantraYaml(exportData.files[yamlPath], checkedFiles);
    }
