---
name: yantra-create-plugin
description: >
  Create new Yantra plugins with the current alpha SDK/runtime. Use when
...
# Create a Yantra Plugin

Use this skill when the task is to create, scaffold, or document a Yantra plugin.
...
node packages/plugins/create-yantra-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the Yantra repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.yantra-sdk/`:

```bash
pnpm --filter @yantra/create-yantra-plugin build
node packages/plugins/create-yantra-plugin/dist/index.js @acme/plugin-name \
...
- is installed into Yantra from an absolute local path during development
