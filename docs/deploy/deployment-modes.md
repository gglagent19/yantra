...
Yantra supports two runtime modes with different security profiles.
...
```sh
pnpm yantraai allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, Yantra emits a one-time claim URL at startup:
...
```sh
YANTRA_DEPLOYMENT_MODE=authenticated pnpm yantraai run
```
