...
Build a custom adapter to connect Yantra to any agent runtime.
...
|---|---|---|
| Source | Inside `yantra-fork` | Separate npm package |
| Distribution | Ships with Yantra | Independent npm publish |
...
For most cases, **build an external adapter plugin**. It's cleaner, independently versioned, and doesn't require modifying Yantra's source. See [External Adapters](/adapters/external-adapters) for the full guide.
...
1. Read config using safe helpers (`asString`, `asNumber`, etc.) from `@yantra/adapter-utils/server-utils`
2. Build environment with `buildYantraEnv(agent)` plus context vars
...
|--------|--------|---------|
| `runChildProcess(cmd, opts)` | `@yantra/adapter-utils/server-utils` | Spawn with timeout, grace, streaming |
| `buildYantraEnv(agent)` | `@yantra/adapter-utils/server-utils` | Inject `YANTRA_*` env vars |
| `renderTemplate(tpl, data)` | `@yantra/adapter-utils/server-utils` | `{{variable}}` substitution |
| `asString(v)` | `@yantra/adapter-utils` | Safe config value extraction |
| `asNumber(v)` | `@yantra/adapter-utils` | Safe number extraction |
...
For built-in adapters registered in Yantra's source:
...
`format-event.ts` — pretty-prints stdout for `yantraai run --watch` using `picocolors`.
...
Make Yantra skills discoverable to your agent runtime without writing to the agent's working directory:
