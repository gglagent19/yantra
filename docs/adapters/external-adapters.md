...
summary: Build, package, and distribute adapters as plugins without modifying Yantra source
---

Yantra supports external adapter plugins that can be installed from npm packages or local directories. External adapters work exactly like built-in adapters — they execute agents, parse output, and render transcripts — but they live in their own package and don't require changes to Yantra's source code.

## Built-in vs External

| | Built-in | External |
|---|---|---|
| Source location | Inside `yantra-fork/packages/adapters/` | Separate npm package or local directory |
...
| Distribution | Ships with Yantra | Published to npm or linked via `file:` |
| Updates | Requires Yantra release | Independent versioning |
...
```json
{
  ...
  "dependencies": {
    "@yantra/adapter-utils": "^2026.325.0",
    "picocolors": "^1.1.0"
  },
  "devDependencies": {
    "@types/node": "^22.0.0",
...
| `yantra.adapterUiParser` | Contract version for the UI parser (`"1.0.0"`) |
...
```ts
import type { ServerAdapterModule } from "@yantra/adapter-utils";
...
export async function execute(
  ...
  // 2. Build environment with Yantra vars injected
  const env = buildYantraEnv(agent);
...
#### Available Helpers from `@yantra/adapter-utils`

| Helper | Purpose |
|--------|---------|
| `runChildProcess(command, opts)` | Spawn a child process with timeout, grace period, and streaming callbacks |
| `buildYantraEnv(agent)` | Inject `YANTRA_*` environment variables |
...
```sh
# Via the Yantra UI
# Settings → Adapters → Install from npm → "my-yantra-adapter"

# Or via API
curl -X POST http://localhost:3102/api/adapters \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"packageName": "my-yantra-adapter"}'
```
...
Local adapters are symlinked into Yantra's adapter directory. Changes to the source are picked up on server restart.

### Via adapter-plugins.json

For development, you can also edit `~/.yantra/adapter-plugins.json` directly:

```json
[
  {
    "packageName": "my-yantra-adapter",
...
```ts
import type { AdapterSessionCodec } from "@yantra/adapter-utils";
...
Other Yantra users can then install your adapter by package name from the UI or API.
