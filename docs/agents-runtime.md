...
Audience: Operators setting up and running agents in Yantra

## 1. What this system does

Agents in Yantra do not run continuously.  
...
- `hermes_local`: runs your local `hermes` CLI (`hermes-yantra-adapter`)
...
- `droid_local`: runs your local Factory Droid CLI (`@henkey/droid-yantra-adapter`)

For local CLI adapters (`claude_local`, `codex_local`, `opencode_local`, `hermes_local`, `droid_local`), Yantra assumes the CLI is already installed and authenticated on the host machine.
...
Yantra stores session IDs for resumable adapters.
...
Yantra pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. Yantra surfaces this as a warning in environment tests, not a hard error.
