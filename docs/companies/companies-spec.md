...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Yantra.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.yantra.yaml`
...
- Yantra must not require extra top-level fields for skill validity
- Yantra-specific extensions must live under `metadata.yantra` or `metadata.sources`
...
For Yantra, the preferred fidelity extension is:
...
- issue/task Yantra-only metadata
...
- Yantra tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Yantra shape:
...
Additional rules for Yantra exporters:
...
- omit empty and default-valued Yantra fields when possible
...
- Yantra exporters should emit `.yantra.yaml` as a sidecar by default
...
Yantra can map this spec to its runtime model like this:
...
- Yantra extension:
  - `.yantra.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Yantra-specific fidelity

Inline Yantra-only metadata that must live inside a shared markdown file should use:

- `metadata.yantra`

That keeps the base format broader than Yantra.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Yantra.

## 20. Cutover

Yantra should cut over to this markdown-first package model as the primary portability format.

`yantra.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Yantra, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
