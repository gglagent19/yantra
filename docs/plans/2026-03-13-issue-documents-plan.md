...
Add first-class **documents** to Yantra as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Yantra workflow/docs.
...
Update the Yantra agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/yantra/SKILL.md`
