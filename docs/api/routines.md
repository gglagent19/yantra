...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-Yantra-Signature` + `X-Yantra-Timestamp` header pair matching the trigger's signing mode.
