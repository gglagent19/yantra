# ClipHub: Marketplace for Yantra Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Yantra configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Yantra-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Yantra company (agents, projects, governance) |
...
A complete Yantra company configuration:
...
Single-agent configurations designed to plug into a Yantra org:
...
Portable skill files that any Yantra agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Yantra's skill loading system
...
interface Listing {
  ...
  yantraVersionMin: string;     // Minimum Yantra version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Yantra company
...
| `POST` | `/api/listings/:id/install` | Install to Yantra company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Yantra company (or create new)
  → ClipHub API calls Yantra API to:
  ...
  → Redirect to Yantra dashboard with new team running
```
...
### 5.3 Creator: Export from Yantra → Publish

```
Running Yantra company → "Export as Blueprint" (CLI or UI)
  → Yantra exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Yantra brand blue/purple)
...
| Install flow | One-click deploy to Yantra company |
...
{
  "targetCompanyId": "uuid",         // Existing Yantra company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `yantra-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Yantra
- **Backend**: Node.js API (or extend Yantra server)
- **Database**: Postgres (can share Yantra's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Yantra auth (or OAuth2)

### 9.2 Integration with Yantra

ClipHub can be:
- **Option A**: A separate app that calls Yantra's API to install blueprints
- **Option B**: A built-in section of the Yantra UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Yantra UI and API.
...
- [ ] Install flow: blueprint → Yantra company
...
- [ ] "Export from Yantra" CLI command
