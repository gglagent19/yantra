...
Yantra companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .yantra.yaml     # Adapter config, env inputs, routines
```
...
- **.yantra.yaml** holds Yantra-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
yantraai company export <company-id> --out ./my-export
```
...
yantraai company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
yantraai company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
yantraai company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.yantra.yaml`
...
```sh
# From a local folder
yantraai company import ./my-export

# From a GitHub URL
yantraai company import https://github.com/org/repo

# From a GitHub subfolder
yantraai company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
yantraai company import org/repo
yantraai company import org/repo/companies/acme
```
...
If `--target` is not specified, Yantra infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
yantraai company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
yantraai company import org/company-templates/engineering-team \
  --target new \
...
```sh
yantraai company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
yantraai company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
yantraai company import ./package \
  --target new \
  --yes \
  --json
```
...
Yantra supports several GitHub URL formats:
