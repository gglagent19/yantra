...
This guide documents the intended runtime model for projects, execution workspaces, and issue runs in Yantra.
...
- Yantra does not automatically start or stop these runtime services as part of issue execution.
- Yantra also does not automatically restart workspace runtime services on server boot.
...
2. Yantra realizes the effective execution workspace, including creating or reusing a worktree when needed.
3. Yantra persists execution-workspace metadata such as paths, refs, and provisioning settings.
