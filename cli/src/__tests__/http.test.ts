...
import { ApiConnectionError, ApiRequestError, YantraApiClient } from "../client/http.js";

describe("YantraApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new YantraApiClient({
    ...
    expect(headers["x-yantra-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new YantraApiClient({ apiBase: "http://localhost:3100" });
