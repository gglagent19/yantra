...
describe("cli telemetry", () => {
  ...
  it("respects telemetry.enabled=false from the config file", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "yantra-cli-telemetry-"));
    const configPath = makeConfigPath(root, false);
    process.env.YANTRA_HOME = path.join(root, "home");
    process.env.YANTRA_INSTANCE_ID = "telemetry-test";
