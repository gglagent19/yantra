...
import { ensureAgentJwtSecret, loadYantraEnvFile, mergeYantraEnvEntries, readYantraEnvEntries, resolveYantraEnvFile } from "../config/env.js";
import { expandHomePrefix } from "../config/home.js";
import type { YantraConfig } from "../config/schema.js";
import { readConfig, resolveConfigPath, writeConfig } from "../config/store.js";
import { printYantraCliBanner } from "../utils/banner.js";
...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.YANTRA_CONFIG;
...
const WORKTREE_NAME_PREFIX = "yantra-";
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.YANTRA_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}

function resolveWorktreeStartPoint(explicit?: string): string | undefined {
  return explicit ?? nonEmpty(process.env.YANTRA_WORKTREE_START_POINT) ?? undefined;
}
...
function createConfiguredStorageFromYantraConfig(config: YantraConfig): ConfiguredStorage {
...
function openConfiguredStorage(configPath: string): ConfiguredStorage {
  ...
  return createConfiguredStorageFromYantraConfig(config);
}
...
function resolveRepoManagedWorktreesRoot(cwd: string): string | null {
  ...
  const marker = `${path.sep}.yantra${path.sep}worktrees${path.sep}`;
  const index = normalized.indexOf(marker);
  if (index === -1) return null;
  const repoRoot = normalized.slice(0, index);
  return path.resolve(repoRoot, ".yantra", "worktrees");
}
...
function collectClaimedWorktreePorts(homeDir: string, currentInstanceId: string, cwd: string): {
  ...
  if (repoManagedWorktreesRoot && existsSync(repoManagedWorktreesRoot)) {
    for (const entry of readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const configPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".yantra", "config.json");
      if (existsSync(configPath)) {
        configPaths.add(configPath);
      }
    }
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.yantra"));
  const sourceInstanceId = sanitizeWorktreeInstanceId(opts.fromInstance ?? "default");
  return path.resolve(sourceHome, "instances", sourceInstanceId, "config.json");
}

function resolveSourceConnectionString(config: YantraConfig, envEntries: Record<string, string>, portOverride?: number): string {
  ...
  return `postgres://yantra:yantra@127.0.0.1:${port}/yantra`;
}

export function copySeededSecretsKey(input: {
  sourceConfigPath: string;
  sourceConfig: YantraConfig;
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.YANTRA_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.YANTRA_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.YANTRA_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.YANTRA_SECRETS_MASTER_KEY_FILE) : null);
...
async function seedWorktreeDatabase(input: {
  sourceConfigPath: string;
  sourceConfig: YantraConfig;
  targetConfig: YantraConfig;
  targetPaths: WorktreeLocalPaths;
  instanceId: string;
  seedMode: WorktreeSeedMode;
}): Promise<SeedWorktreeDatabaseResult> {
  const seedPlan = resolveWorktreeSeedPlan(input.seedMode);
  const sourceEnvFile = resolveYantraEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readYantraEnvEntries(sourceEnvFile);
  ...
  try {
    ...
    const adminConnectionString = `postgres://yantra:yantra@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "yantra");
    const targetConnectionString = `postgres://yantra:yantra@127.0.0.1:${targetHandle.port}/yantra`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  const sourceEnvEntries = readYantraEnvEntries(resolveYantraEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.YANTRA_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.YANTRA_AGENT_JWT_SECRET);
  mergeYantraEnvEntries(
    {
      ...buildWorktreeEnvEntries(paths, branding),
      ...(existingAgentJwtSecret ? { YANTRA_AGENT_JWT_SECRET: existingAgentJwtSecret } : {}),
    },
    paths.envPath,
  );
  ensureAgentJwtSecret(paths.configPath);
  loadYantraEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run Yantra inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}

export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  printYantraCliBanner();
  p.intro(pc.bgCyan(pc.black(" yantraai worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printYantraCliBanner();
  p.intro(pc.bgCyan(pc.black(" yantraai worktree:make ")));
...
function toMergeSourceChoices(cwd: string): MergeSourceChoice[] {
  ...
  return parseGitWorktreeList(cwd).map((entry) => {
    ...
    return {
      ...
      branch: entry.branch,
      branchLabel,
      hasYantraConfig: existsSync(path.resolve(worktreePath, ".yantra", "config.json")),
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printYantraCliBanner();
  p.intro(pc.bgCyan(pc.black(" yantraai worktree:cleanup ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolveYantraEnvFile(configPath);
  const envEntries = readYantraEnvEntries(envPath);
  const out = {
    YANTRA_CONFIG: configPath,
    ...(envEntries.YANTRA_HOME ? { YANTRA_HOME: envEntries.YANTRA_HOME } : {}),
    ...(envEntries.YANTRA_INSTANCE_ID ? { YANTRA_INSTANCE_ID: envEntries.YANTRA_INSTANCE_ID } : {}),
    ...(envEntries.YANTRA_CONTEXT ? { YANTRA_CONTEXT: envEntries.YANTRA_CONTEXT } : {}),
    ...envEntries,
  };
...
function resolveAttachmentLookupStorages(input: {
  ...
  const orderedConfigPaths = [
    ...
    ...toMergeSourceChoices(process.cwd())
      .filter((choice) => choice.hasYantraConfig)
      .map((choice) => path.resolve(choice.worktree, ".yantra", "config.json")),
  ];
...
async function openConfiguredDb(configPath: string): Promise<OpenDbHandle> {
  ...
  const envEntries = readYantraEnvEntries(resolveYantraEnvFile(configPath));
    ...
    if (migrationState.status !== "upToDate") {
      const pending =
        migrationState.reason === "pending-migrations"
          ...
          : "";
      throw new Error(
        `Database for ${configPath} is not up to date.${pending} Run \`pnpm db:migrate\` (or start Yantra once) before using worktree merge history.`,
      );
    }
...
export async function worktreeListCommand(opts: WorktreeListOptions): Promise<void> {
  ...
  for (const choice of choices) {
    const flags = [
      ...
      choice.hasYantraConfig ? "yantra" : "no-yantra-config",
...
function resolveEndpointFromChoice(choice: MergeSourceChoice): ResolvedWorktreeEndpoint {
  ...
  return {
    ...
    configPath: path.resolve(choice.worktree, ".yantra", "config.json"),
...
function resolveWorktreeEndpointFromSelector(
  ...
  if (existsSync(directPath)) {
    ...
    const configPath = path.resolve(directPath, ".yantra", "config.json");
    if (!existsSync(configPath)) {
      throw new Error(`Resolved worktree path ${directPath} does not contain .yantra/config.json.`);
    }
    return {
      rootPath: directPath,
      configPath,
  ...
  if (!matched.hasYantraConfig && !matched.isCurrent) {
    throw new Error(`Resolved worktree "${selector}" does not look like a Yantra worktree.`);
  }
...
async function promptForSourceEndpoint(excludeWorktreePath?: string): Promise<ResolvedWorktreeEndpoint> {
  ...
  const currentEndpoint = resolveCurrentEndpoint();
  const choices = toMergeSourceChoices(process.cwd())
    .filter((choice) => choice.hasYantraConfig || choice.isCurrent)
  ...
  if (choices.length === 0) {
    throw new Error("No Yantra worktrees were found. Run `yantraai worktree:list` to inspect the repo worktrees.");
  }
...
export async function worktreeMergeHistoryCommand(sourceArg: string | undefined, opts: WorktreeMergeHistoryOptions): Promise<void> {
  ...
  if (path.resolve(sourceEndpoint.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target Yantra configs are the same. Choose different --from/--to worktrees.");
  }
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local Yantra instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated Yantra instance inside it")
    .argument("<name>", "Worktree name — auto-prefixed with yantra- if needed (created at ~/yantra-NAME)")
    .option("--start-point <ref>", "Remote ref to base the new branch on (env: YANTRA_WORKTREE_START_POINT)")
    .option("--instance <id>", "Explicit isolated instance id")
    .option("--home <path>", `Home root for worktree instances (env: YANTRA_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source YANTRA_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local Yantra instance")
  ...
  program
    ...
    .description("List git worktrees visible from this repo and whether they look like Yantra worktrees")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with yantra- if needed")
