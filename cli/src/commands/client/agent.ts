...
import type { Agent } from "@yantra/shared";
import {
  removeMaintainerOnlySkillSymlinks,
  resolveYantraSkillsDir,
...
function buildAgentEnvExports(input: {
  ...
  return [
    `export YANTRA_API_URL='${escaped(input.apiBase)}'`,
    `export YANTRA_COMPANY_ID='${escaped(input.companyId)}'`,
    `export YANTRA_AGENT_ID='${escaped(input.agentId)}'`,
    `export YANTRA_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local Yantra skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing Yantra skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveYantraSkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local Yantra skills directory. Expected ./skills in the repo checkout.",
              );
            }
