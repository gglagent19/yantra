...
import { ApiRequestError, YantraApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to Yantra config file")
    .option("-d, --data-dir <path>", "Yantra data directory root (isolates state from ~/.yantra)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Yantra API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.YANTRA_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.YANTRA_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.YANTRA_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set YANTRA_COMPANY_ID, or set context profile companyId via `yantraai context set`.",
    );
  }

  const api = new YantraApiClient({
    apiBase,
    apiKey,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.YANTRA_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.YANTRA_SERVER_PORT || "");
