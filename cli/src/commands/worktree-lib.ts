...
export const DEFAULT_WORKTREE_HOME = "~/.yantra-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".yantra");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: YantraConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "yantra",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    YANTRA_HOME: paths.homeDir,
    YANTRA_INSTANCE_ID: paths.instanceId,
    YANTRA_CONFIG: paths.configPath,
    YANTRA_CONTEXT: paths.contextPath,
    YANTRA_IN_WORKTREE: "true",
    ...(branding?.name ? { YANTRA_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { YANTRA_WORKTREE_COLOR: branding.color } : {}),
  };
}
