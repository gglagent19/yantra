...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@yantra/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://yantra:yantra@127.0.0.1:${port}/yantra`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" yantra db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveYantraInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "yantra";
