...
function defaultConfig(): YantraConfig {
...
export async function configure(opts: {
  ...
  p.intro(pc.bgCyan(pc.black(" yantra configure ")));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `yantraai onboard` first.");
    p.outro("");
    return;
  }

  let config: YantraConfig;
  try {
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because YANTRA_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }
