...
const JWT_SECRET_ENV_KEY = "YANTRA_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by Yantra CLI commands",
...
export function resolveYantraEnvFile(configPath?: string): string {
...
export function loadYantraEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeYantraEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readYantraEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeYantraEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeYantraEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readYantraEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeYantraEnvEntries(next, filePath);
  return next;
}
