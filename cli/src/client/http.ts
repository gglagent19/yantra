...
export class YantraApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-yantra-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the Yantra API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the Yantra server is not running, the configured URL is wrong, or the request is being blocked before it reaches Yantra.",
    "",
    "Try:",
    "- Start Yantra with `pnpm dev` or `pnpm yantraai run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If Yantra is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`YANTRA_API_URL\`.`,
  );
  return lines.join("\n");
}
