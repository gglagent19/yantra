/**
 * esbuild configuration for building the yantraai CLI for npm.
 *
 * Bundles all workspace packages (@yantra/*) into a single file.
...
// to npm and resolved at runtime (e.g. @yantra/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@yantra/")) {
      externals.add(name);
    }
  }
