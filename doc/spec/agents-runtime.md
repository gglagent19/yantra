...
For `claude_local` and `codex_local`, Yantra assumes the CLI is already installed and authenticated on the host machine.
...
Yantra stores resumable session state per `(agent, taskKey, adapterType)`.
