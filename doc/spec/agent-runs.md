...
This spec defines how Yantra actually runs agents while staying runtime-agnostic.
...
1. Yantra is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Yantra manages lifecycle, and on exit Yantra parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `yantra.skill` (shared Yantra skill text block)
...
2. Preferred transport is env vars (`YANTRA_*`) injected at runtime.
