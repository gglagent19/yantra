...
Low-level reference for how Yantra packages are prepared and published to npm.
...
Yantra no longer uses release branches or Changesets for publishing.

## Why the CLI needs special packaging

The CLI package, `yantraai`, imports code from workspace packages such as:
...
### Manual first publish for `@yantra/ui`
...
npm view @yantra/ui version

# make sure the dist payload is fresh
pnpm --filter @yantra/ui build
...
- If `npm view @yantra/ui version` already returns the same version that is in [`ui/package.json`](../ui/package.json), do not republish. Bump the version or use the normal repo-wide release flow in [`scripts/release.sh`](../scripts/release.sh).
...
- For an organization-scoped package like `@yantra/ui`, the `yantraai` npm organization must exist and the publisher must be a member with permission to publish to that scope.
