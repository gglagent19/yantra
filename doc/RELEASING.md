...
Maintainer runbook for shipping Yantra across npm, GitHub, and the website-facing changelog surface.
...
Yantra uses calendar versions that still fit semver syntax:
...
2. **npm** — `yantraai` and public workspace packages are published
...
```bash
npx yantraai@canary onboard
# or
npx yantraai@canary onboard --data-dir "$(mktemp -d /tmp/yantra-canary.XXXXXX)"
```
...
[Run the action here](https://github.com/yantraai/yantra/actions/workflows/release.yml)
...
```bash
YANTRAAI_VERSION=canary ./scripts/docker-onboard-smoke.sh
```

For the current stable:

```bash
YANTRAAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

Useful isolated variants:

```bash
HOST_PORT=3232 DATA_DIR=./data/release-smoke-canary YANTRAAI_VERSION=canary ./scripts/docker-onboard-smoke.sh
HOST_PORT=3233 DATA_DIR=./data/release-smoke-stable YANTRAAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

Automated browser smoke is also available:

```bash
gh workflow run release-smoke.yml -f yantra_version=canary
gh workflow run release-smoke.yml -f yantra_version=latest
```

Minimum checks:

- `npx yantraai@canary onboard` installs
