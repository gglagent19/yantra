# Yantra Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Yantra's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Yantra app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Yantra HTTP APIs directly.
...
Yantra plugin design is based on the following assumptions:

1. Yantra is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Yantra business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Yantra core.
...
2. Let plugins add major capabilities without editing Yantra core.
...
4. Depend on project-local plugin folders such as `.yantra/plugins`.
...
The single Yantra deployment an operator installs and controls.

### 5.2 Company

A first-class Yantra business object inside the instance.
...
A trusted in-process extension loaded directly by Yantra core.
...
An installable instance-wide extension package loaded through the Yantra plugin runtime.
...
Yantra has two extension classes.
...
Yantra already has a concrete workspace model:
...
Plugins live under the Yantra instance directory.

Suggested layout:

- `~/.yantra/instances/default/plugins/package.json`
- `~/.yantra/instances/default/plugins/node_modules/`
- `~/.yantra/instances/default/plugins/.cache/`
- `~/.yantra/instances/default/data/plugins/<plugin-id>/`
...
Yantra should add CLI commands:

- `pnpm yantraai plugin list`
- `pnpm yantraai plugin install <package[@version]>`
- `pnpm yantraai plugin uninstall <plugin-id>`
- `pnpm yantraai plugin upgrade <plugin-id> [version]`
- `pnpm yantraai plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@yantra/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface YantraPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that Yantra agents can use during runs.
...
- Yantra server starts one worker process per installed plugin
...
Receives one typed Yantra domain event.
...
export function definePlugin(definition: PluginDefinition): YantraPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: YantraPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@yantra/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@yantra/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@yantra/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@yantra/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@yantra/plugin-sdk/ui`
...
The `@yantra/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Yantra secret provider system rather than a plain text input
...
1. Core Yantra data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Yantra objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Yantra product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Yantra later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Yantra secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm yantraai plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Yantra server. This is a normative requirement, not optional.
...
### 27.1 `@yantra/plugin-test-harness`
...
```ts
import { createTestHarness } from "@yantra/plugin-test-harness";
...
For developing a plugin against a running Yantra instance:

- The operator installs the plugin from a local path: `pnpm yantraai plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-yantra-plugin`) that scaffolds:

- `package.json` with correct `yantraPlugin` keys
...
- `@yantra/plugin-workspace-files`
- `@yantra/plugin-terminal`
- `@yantra/plugin-git`
- `@yantra/plugin-linear`
- `@yantra/plugin-github-issues`
- `@yantra/plugin-grafana`
- `@yantra/plugin-runtime-processes`
- `@yantra/plugin-stripe`
...
- `@yantra/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@yantra/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@yantra/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@yantra/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@yantra/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@yantra/plugin-sdk/ui`
...
- `@yantra/plugin-test-harness`
- `create-yantra-plugin` starter template
...
Yantra should not implement a generic in-process hook bag modeled directly after local coding tools.

Yantra should implement:
...
That is the complete target design for the Yantra plugin system.
