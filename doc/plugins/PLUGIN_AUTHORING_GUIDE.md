...
This guide describes the current, implemented way to create a Yantra plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Yantra app.
...
```bash
pnpm --filter @yantra/create-yantra-plugin build
node packages/plugins/create-yantra-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the Yantra repo:

```bash
pnpm --filter @yantra/create-yantra-plugin build
node packages/plugins/create-yantra-plugin/dist/index.js @yourscope/plugin-name \
  --output /absolute/path/to/plugin-repos \
  --sdk-path /absolute/path/to/yantra/packages/plugins/sdk
```
...
Inside this monorepo, the scaffold uses `workspace:*` for `@yantra/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@yantra/plugin-sdk` from the local Yantra checkout into a `.yantra-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into Yantra from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@yantra/plugin-sdk/ui`
