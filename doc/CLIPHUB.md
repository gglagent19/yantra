...
ClipHub is the public registry where people share, discover, and download Yantra company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Yantra what a package registry is to a programming language. Yantra already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Yantra — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Yantra instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Yantra spec. It contains:
...
**Install (fresh start):**
```
yantra install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Yantra instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Yantra, export your company as a template, then publish:

```
yantra export --template my-company
yantra publish cliphub my-company
```
...
ClipHub is a **separate service** from Yantra itself. Yantra is self-hosted; ClipHub is a hosted registry that Yantra instances talk to.
...
| **Yantra CLI** | `yantraai install`, `yantraai publish`, `yantraai cliphub sync` — built into Yantra |
| **Yantra UI** | "Browse ClipHub" panel in the Yantra web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Yantra) |
| Backend | TypeScript + Hono (consistent with Yantra) |
| Database | PostgreSQL |
...
4. Run `yantraai install cliphub:acme/lean-saas-shop`
5. Yantra creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Yantra until it works well
2. Export: `yantraai export --template my-agency`
3. Publish: `yantraai publish cliphub my-agency`
...
3. Install just that agent: `yantraai install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Yantra

ClipHub is **not required** to use Yantra. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Yantra what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `yantraai install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Yantra web UI
- [ ] `yantraai cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Yantra's own philosophy)
