...
Date: 2026-03-13  
Related discussion: https://github.com/yantraai/yantra/discussions/449
...
- local runtime data from the default Yantra instance on 2026-03-13
...
3. **Repeated context reacquisition.** The `yantra` skill tells agents to re-fetch assignments, issue details, ancestors, and full comment threads on every heartbeat. The API does not currently offer efficient delta-oriented alternatives.
...
The `yantra` skill currently tells agents to do this on essentially every heartbeat:
...
- Yantra discovers repo skills from the current checkout, then symlinks them into `$CODEX_HOME/skills` or `~/.codex/skills`.
- If an existing Yantra skill symlink already points at another live checkout, the current implementation skips it instead of repointing it.
- This can leave Codex using stale skill content from a different worktree even after Yantra-side skill changes land.
...
- `skills/yantra/SKILL.md`: 17,441 bytes
- `.agents/skills/create-agent-adapter/SKILL.md`: 31,832 bytes
- `skills/yantra-create-agent/SKILL.md`: 4,718 bytes
...
- Store both:
  ...
  - Yantra-normalized per-run usage
...
For `codex_local`, this also requires isolating the Codex skill home per worktree or teaching Yantra to repoint its own skill symlinks when the source checkout changes. Otherwise prompt and skill improvements in the active worktree may not reach the running agent.
...
Update the `yantra` skill so the default pattern becomes:
...
- For `codex_local`, either:
  ...
  - treat Yantra-owned Codex skill symlinks as repairable when they point at a different checkout
...
4. heartbeat delta APIs + `yantra` skill rewrite
...
6. Rewrite `skills/yantra/SKILL.md` around delta-fetch behavior.
