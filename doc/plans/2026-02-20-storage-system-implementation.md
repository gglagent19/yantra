...
Add a single storage subsystem for Yantra that supports:
...
- API serves attachment bytes through authenticated Yantra endpoints.
...
- Default local path is under instance root: `~/.yantra/instances/<instanceId>/data/storage`.
...
- `yantraai onboard` writes a valid `storage` config block by default.
- `yantraai configure --section storage` can switch between local and s3 modes.
...
- `yantraai doctor` reports actionable storage status.
