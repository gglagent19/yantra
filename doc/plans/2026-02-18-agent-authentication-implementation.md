...
- New config/env settings:
  - `YANTRA_AGENT_JWT_SECRET`
  - `YANTRA_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `YANTRA_AGENT_JWT_ISSUER` (default: `yantra`)
  - `YANTRA_AGENT_JWT_AUDIENCE` (default: `yantra-api`)
...
1. In:
   ...
   inject `YANTRA_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `YANTRA_API_KEY`, do not overwrite it.
- Continue injecting:
  - `YANTRA_AGENT_ID`
  - `YANTRA_COMPANY_ID`
  - `YANTRA_API_URL`
...
- Local adapters authenticate without manual `YANTRA_API_KEY` config.
