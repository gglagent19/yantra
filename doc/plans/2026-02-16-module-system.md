...
Yantra's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Yantra's API, UI, and data model | Yes |
...
modules/
  observability/
    yantra.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@yantra/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@yantra/core";
...
```
1. Scan modules/ directory for yantra.module.json manifests
...
```jsonc
// yantra.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@yantra/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@yantra/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:yantra/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.yantra.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm yantraai store list                    # browse available modules and templates
pnpm yantraai store install <module-id>     # install a module
pnpm yantraai store import <template-id>    # import a company template
pnpm yantraai store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Yantra instance | `server:started` |
...
4. **Module config** — `yantra.config.json` with per-module config, env var interpolation
...
10. **`@yantra/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
