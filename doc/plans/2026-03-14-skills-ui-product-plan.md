...
This document defines the product and UI plan for skill management in Yantra.
...
- Yantra company import/export can include skills as package content
...
3. The UI must always tell the truth about what Yantra knows:
   - desired state in Yantra
...
Yantra should treat skills at two scopes:
...
- created locally inside Yantra later
...
7. “Understand what Yantra knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a Yantra-specific package layout
...
This matters because Codex and similar adapters may already have local skills that Yantra did not install.

These should be clearly marked:

- external
- not managed by Yantra
...
- `desired_only`: Yantra wants it, adapter does not show it yet
- `external`: adapter has it but Yantra does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, Yantra only tracks desired state
...
- Yantra can still track desired skills
...
- `.yantra.yaml` may add Yantra-specific fidelity, but should not replace the base shortname association model
...
- `.yantra.yaml` may add Yantra-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
Yantra should have a company-scoped skill model or managed package model representing:
...
Yantra should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too Yantra-specific.
...
That gives Yantra one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
