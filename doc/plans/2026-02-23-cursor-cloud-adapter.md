...
This document defines the V1 design for a Yantra adapter that integrates with
...
4. synthesized stdout events for Yantra UI/CLI
...
3. **Agent callback auth to Yantra** uses a bootstrap exchange flow (no long-lived Yantra key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Yantra endpoints, not full SKILL.md prompt injection.
...
- `yantraPublicUrl` (optional override; else `YANTRA_PUBLIC_URL` env)
...
## Yantra Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `YANTRA_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.yantraPublicUrl`
2. `process.env.YANTRA_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Yantra credentials in prompt text.

Flow:

1. Before launch/follow-up, Yantra mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Yantra validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Yantra API calls.
...
1. Prompt includes a compact instruction to fetch skills from Yantra.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/yantra`
   - `GET /api/skills/yantra-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <yantraPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Yantra run context.
...
5. `yantraPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from Yantra skill endpoints |
| Yantra auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Yantra context.
3. Add richer PR metadata surfacing in Yantra UI.
