...
`PAP-447` asks how Yantra should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Yantra instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Yantra:

- some operators use Yantra to manage Yantra and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Yantra
- Claude and Codex expose different built-in affordances, so Yantra should not overfit to one tool

## Core Product Decision

Yantra should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Yantra
...
- Yantra should not assume that every adapter can see or use a host filesystem path directly
...
If Yantra makes worktrees agent-first, it will blur:
...
- Yantra core resolves and records execution workspace state
...
That gives Yantra room to support:
...
- Yantra handles workspace runtime services under the hood using project/workspace policy
...
Once Yantra is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Yantra cannot infer
...
- when issue moves to done, should Yantra auto-commit?
- should it auto-push?
...
Yantra already has the right foundation for a project-first model.
...
For server-spawned local adapters, Yantra already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Yantra should mirror directly

Implication:

- for `codex_local`, Yantra should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Yantra should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Yantra should separate:
...
Yantra should be able to express intentions such as:
...
These are not specific to Yantra. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".yantra/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Yantra-specific `devServer` flag.
...
Yantra should distinguish between:
...
Yantra should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Yantra should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Yantra
...
For runtime services specifically, Yantra should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Yantra-only dev-server flags
...
3. Inject resolved execution workspace details into `context.yantraWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Yantra instance can be reused through env/config injection when appropriate
...
- Yantra can decide whether to reuse or start a fresh service deterministically
...
3. Test against Yantra-on-Yantra workflows first.
...
7. Workspace runtime services are modeled generically, not as Yantra-specific dev-server toggles.
