...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented Yantra plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside Yantra.
...
- package: `@yantra/plugin-kitchen-sink-example`
- plugin id: `yantra.kitchen-sink-example` or `yantra-kitchen-sink-example`

Recommendation: use `yantra-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core Yantra event such as `issue.created`
...
Recommendation: default to a Yantra-local endpoint or a stable public echo endpoint to avoid flaky docs.
