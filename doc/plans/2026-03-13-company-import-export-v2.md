...
This document defines the next-stage plan for Yantra company import/export.

The core shift is:

- move from a Yantra-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside Yantra.
...
Yantra already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just Yantra
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `yantra.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.yantra.yaml` sidecar for high-fidelity Yantra-specific details
...
11. adapter-aware skill sync surfaces so Yantra can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point Yantra at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a Yantra sidecar.
...
Yantra must not redefine `SKILL.md`.
...
- Yantra-specific fidelity lives in `.yantra.yaml`
- Yantra may resolve and install `SKILL.md` packages, but it must not require a Yantra-only skill format
...
### 5.4 Base Package Vs Yantra Extension
...
- base package:
  ...
  - no Yantra-only runtime fields by default
- Yantra extension:
  - `.yantra.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by Yantra tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`yantra.manifest.json` is not part of the future package direction.
...
Yantra import/export should support these entity kinds:
...
- Yantra can treat “this set of agents came from team package X” as the imported-team model
...
Yantra should support source references in package metadata with:
...
Yantra should define a new adapter surface area around skills:
...
Planned Yantra behavior:

- if an adapter supports read, Yantra should show current skills in the UI
- if an adapter supports write, Yantra should let the user enable/disable imported skills
- if an adapter supports sync, Yantra should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.yantra.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.yantra.yaml` alongside the base package
...
## 10. Storage Model Inside Yantra
...
Yantra should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `yantraai company export <company-id> --out <path>`
- `yantraai company import <path-or-url> --dry-run`
- `yantraai company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in Yantra storage, or only referenced at import time?
...
3. Should Yantra support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps Yantra aligned with:
