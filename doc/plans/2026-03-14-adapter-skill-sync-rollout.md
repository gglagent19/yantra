...
This document defines the rollout plan for adapter-wide skill support in Yantra.
...
- Yantra stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

Yantra currently has these adapters:
...
1. Desired skills live in Yantra for every adapter.
...
These adapters have a stable local skills directory that Yantra can read and manage.
...
These adapters do not have a meaningful Yantra-owned persistent install state.
...
- desired skills stored in Yantra
...
- runtime already injects Yantra skills into `~/.cursor/skills`
...
- runtime already injects Yantra skills into `~/.gemini/skills`
...
- runtime already injects Yantra skills into `~/.pi/agent/skills`
...
- Yantra can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects Yantra skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- Yantra may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed Yantra skills that are clearly marked as Yantra-managed
...
- Yantra stores desired skills only
...
- avoid destructive wording unless Yantra can prove a skill is Yantra-managed
...
That gets Yantra from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
