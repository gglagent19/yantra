...
Today release smoke testing for published Yantra packages is manual and shell-driven:
...
- runs `yantraai@${YANTRAAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@yantra.local`
- password: `yantra-smoke-password`
...
- the suite passes locally against both:
  - `YANTRAAI_VERSION=canary`
  - `YANTRAAI_VERSION=latest`
...
1. A published `yantraai@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `yantraai@latest` Docker install can be smoke-tested by Playwright in CI.
