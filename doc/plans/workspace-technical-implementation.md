...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/yantra-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `YANTRA_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `YANTRA_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - Yantra may create host-local worktrees, processes, and paths
- `cloud`
  - Yantra should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `yantra | github | vercel | s3 | custom`
...
- use `YANTRA_EXECUTION_TOPOLOGY` as a defaulting guardrail
