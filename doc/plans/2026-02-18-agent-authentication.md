...
Agents need API keys to authenticate with Yantra. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Yantra
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Yantra generates a token and passes it directly to the agent
...
`YANTRA_API_URL`, `YANTRA_AGENT_ID`, `YANTRA_COMPANY_ID`. We need to
add a `YANTRA_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Yantra CLI
...
```
yantra auth login
# Opens browser -> user confirms -> token stored at ~/.yantra/credentials
```
...
managed by the Yantra server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Yantra.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Yantra SKILL.md (or a link to it)
   - What information Yantra needs from the agent (e.g. webhook URL, adapter
...
5. Yantra stores the pending registration.
...
7. On approval, Yantra provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Yantra issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.yantra.ing/skills/yantra/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Yantra should send you heartbeats",
...
1. Generate a Yantra invite link for the company.
...
4. A Yantra company member approves the new agent.
5. Yantra begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `YANTRA_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `yantraai auth login` for developer-managed remote agents.                                      |
