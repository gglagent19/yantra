...
Yantra needs to support two very different but equally valid ways of working:
...
Today, Yantra already has the beginnings of this model:
...
- making Yantra feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Yantra users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Yantra deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Yantra into a full code review product
...
This object must be recorded explicitly so that Yantra can:
...
Yantra should treat PRs as a type of work product linked back to:
...
Yantra already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Yantra must be a first-class deployment mode

Yantra cannot assume that it is running on the same machine as the code.

In cloud deployments, Yantra may:
...
- `Runtime service`: a process or service Yantra owns or tracks for a workspace
...
It also lets cloud-hosted Yantra deployments point at codebases and remotes without pretending the Yantra host has direct filesystem access.
...
Without an explicit `execution workspace` record, Yantra has nowhere stable to attach:
...
Yantra needs a single place to show:
...
- Yantra-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Yantra owning the execution host
...
## 1. Yantra host and execution host must be decoupled
...
without the Yantra host itself running local git or local preview processes.
...
Yantra should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Yantra.
...
Yantra should keep workspace automation centered on a small set of hooks:
...
Yantra should provide the equivalent visibility in the project `Code` surface:
...
Yantra should treat execution workspaces as the natural home for:
...
Yantra should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Yantra should continue treating cleanup policy as part of the core workspace model:
...
Yantra should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Yantra host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Yantra should borrow Worktrunk's good execution patterns while keeping the broader Yantra model:
...
That keeps Yantra focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Yantra deployments remain first-class

That is a better fit for Yantra than either extreme:
