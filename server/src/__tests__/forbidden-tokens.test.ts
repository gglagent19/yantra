...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "yantra", LOGNAME: "yantra", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "yantra" }),
      },
    );

    expect(tokens).toEqual(["yantra", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "yantra" }, {
        userInfo: () => ({ username: "yantra" }),
      });

      expect(tokens).toEqual(["yantra", "custom-token"]);
    } finally {
