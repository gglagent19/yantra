...
import { isCursorUnknownSessionError, parseCursorJsonl } from "@yantra/adapter-cursor-local/server";
import { parseCursorStdoutLine } from "@yantra/adapter-cursor-local/ui";
import { printCursorStreamEvent } from "@yantra/adapter-cursor-local/cli";
...
describe("cursor ui stdout parser", () => {
  ...
  it("compacts shellToolCall and shell tool result for run log", () => {
    const ts = "2026-03-05T00:00:00.000Z";
    const longCommand = "curl -s -X POST \"$YANTRA_API_URL/api/issues/abc/checkout\" -H \"Authorization: Bearer $YANTRA_API_KEY\"";
