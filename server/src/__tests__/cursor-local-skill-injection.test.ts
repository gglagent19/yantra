...
import { ensureCursorSkillsInjected } from "@yantra/adapter-cursor-local/server";
...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing Yantra skills into Cursor skills home", async () => {
    const skillsDir = await makeTempDir("yantra-cursor-skills-src-");
    const skillsHome = await makeTempDir("yantra-cursor-skills-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);

    await createSkillDir(skillsDir, "yantra");
    await createSkillDir(skillsDir, "yantra-create-agent");
    ...
    const injectedA = path.join(skillsHome, "yantra");
    const injectedB = path.join(skillsHome, "yantra-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "yantra")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "yantra-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "yantra"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "yantra-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("yantra-cursor-preserve-src-");
    const skillsHome = await makeTempDir("yantra-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "yantra");
    ...
    expect((await fs.lstat(path.join(skillsHome, "yantra-create-agent"))).isSymbolicLink()).toBe(true);
  });

  it("logs per-skill link failures and continues without throwing", async () => {
    const skillsDir = await makeTempDir("yantra-cursor-fail-src-");
    const skillsHome = await makeTempDir("yantra-cursor-fail-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
