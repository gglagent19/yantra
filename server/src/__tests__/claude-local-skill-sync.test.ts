...
describe("claude local skill sync", () => {
  ...
  const createAgentKey = "yantraai/yantra/yantra-create-agent";
  ...
  it("defaults to mounting all built-in Yantra skills when no explicit selection exists", async () => {
    ...
    expect(snapshot.entries.find((entry) => entry.key === yantraKey)?.required).toBe(true);
  ...
  it("normalizes legacy flat Yantra skill refs to canonical keys", async () => {
    const snapshot = await listClaudeSkills({
      agentId: "agent-3",
      companyId: "company-1",
      adapterType: "claude_local",
  ...
  it("shows host-level user-installed Claude skills as read-only external entries", async () => {
    const home = await makeTempDir("yantra-claude-user-skills-");
