...
describe("agent local JWT", () => {
  const secretEnv = "YANTRA_AGENT_JWT_SECRET";
  const ttlEnv = "YANTRA_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "YANTRA_AGENT_JWT_ISSUER";
  const audienceEnv = "YANTRA_AGENT_JWT_AUDIENCE";
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "yantra";
    process.env[audienceEnv] = "yantra-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
