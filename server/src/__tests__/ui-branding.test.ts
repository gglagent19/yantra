...
<head>
    <!-- YANTRA_RUNTIME_BRANDING_START -->
    <!-- YANTRA_RUNTIME_BRANDING_END -->
    <!-- YANTRA_FAVICON_START -->
...
describe("ui branding", () => {
  it("detects worktree mode from YANTRA_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ YANTRA_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ YANTRA_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ YANTRA_IN_WORKTREE: "false" })).toBe(false);
  });

  it("resolves name, color, and text color for worktree branding", () => {
    const branding = getWorktreeUiBranding({
      YANTRA_IN_WORKTREE: "true",
      YANTRA_WORKTREE_NAME: "yantra-pr-432",
      YANTRA_WORKTREE_COLOR: "#4f86f7",
    });

    expect(branding.enabled).toBe(true);
    expect(branding.name).toBe("yantra-pr-432");
  ...
  it("renders runtime branding metadata for the ui", () => {
    ...
    expect(meta).toContain('name="yantra-worktree-name"');
    expect(meta).toContain('content="yantra-pr-432"');
    expect(meta).toContain('name="yantra-worktree-color"');
  });
  ...
  it("rewrites the favicon and runtime branding blocks for worktree instances only", () => {
    ...
    expect(branded).toContain('name="yantra-worktree-name"');
    ...
    expect(defaultHtml).not.toContain('name="yantra-worktree-name"');
  });
});
