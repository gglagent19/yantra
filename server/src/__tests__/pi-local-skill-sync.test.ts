...
describe("pi local skill sync", () => {
  ...
  it("reports configured Yantra skills and installs them into the Pi skills home", async () => {
    const home = await makeTempDir("yantra-pi-skill-sync-");
    ...
    const after = await syncPiSkills(ctx, [yantraKey]);
    expect(after.entries.find((entry) => entry.key === yantraKey)?.state).toBe("installed");
    expect((await fs.lstat(path.join(home, ".pi", "agent", "skills", "yantra"))).isSymbolicLink()).toBe(true);
  });

  it("keeps required bundled Yantra skills installed even when the desired set is emptied", async () => {
    const home = await makeTempDir("yantra-pi-skill-prune-");
    cleanupDirs.add(home);
    ...
    await syncPiSkills(configuredCtx, [yantraKey]);
