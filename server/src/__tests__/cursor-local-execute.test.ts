...
import { execute } from "@yantra/adapter-cursor-local/server";
...
describe("cursor execute", () => {
  it("injects yantra env vars and prompt note by default", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "yantra-cursor-execute-"));
    ...
    try {
      ...
      expect(capture.argv).not.toContain("Follow the yantra heartbeat.");
      ...
      expect(capture.prompt).toContain("Yantra runtime note:");
      expect(capture.prompt).toContain("YANTRA_API_KEY");
  ...
  it("passes --mode when explicitly configured", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "yantra-cursor-execute-mode-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "agent");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("injects company-library runtime skills into the Cursor skills home before execution", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "yantra-cursor-execute-runtime-skill-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "agent");
    ...
    const yantraDir = await createSkillDir(runtimeSkillsRoot, "yantra");
