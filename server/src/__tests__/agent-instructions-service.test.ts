...
describe("agent instructions service", () => {
  const originalYantraHome = process.env.YANTRA_HOME;
  const originalYantraInstanceId = process.env.YANTRA_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalYantraHome === undefined) delete process.env.YANTRA_HOME;
    else process.env.YANTRA_HOME = originalYantraHome;
    if (originalYantraInstanceId === undefined) delete process.env.YANTRA_INSTANCE_ID;
    else process.env.YANTRA_INSTANCE_ID = originalYantraInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const yantraHome = await makeTempDir("yantra-agent-instructions-home-");
    const externalRoot = await makeTempDir("yantra-agent-instructions-external-");
    cleanupDirs.add(yantraHome);
    cleanupDirs.add(externalRoot);
    process.env.YANTRA_HOME = yantraHome;
    process.env.YANTRA_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("yantra-agent-instructions-new-external-");
    cleanupDirs.add(yantraHome);
    cleanupDirs.add(externalRoot);
    process.env.YANTRA_HOME = yantraHome;
    process.env.YANTRA_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("yantra-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const yantraHome = await makeTempDir("yantra-agent-instructions-recover-");
    cleanupDirs.add(yantraHome);
    process.env.YANTRA_HOME = yantraHome;
    process.env.YANTRA_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const yantraHome = await makeTempDir("yantra-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("yantra-agent-instructions-stale-root-");
    cleanupDirs.add(yantraHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const yantraHome = await makeTempDir("yantra-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("yantra-agent-instructions-heal-write-stale-");
    cleanupDirs.add(yantraHome);
    cleanupDirs.add(staleRoot);
    process.env.YANTRA_HOME = yantraHome;
    process.env.YANTRA_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const yantraHome = await makeTempDir("yantra-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("yantra-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(yantraHome);
    cleanupDirs.add(staleRoot);
    process.env.YANTRA_HOME = yantraHome;
    process.env.YANTRA_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const yantraHome = await makeTempDir("yantra-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("yantra-agent-instructions-partial-root-");
    cleanupDirs.add(yantraHome);
    cleanupDirs.add(staleRoot);
    process.env.YANTRA_HOME = yantraHome;
    process.env.YANTRA_INSTANCE_ID = "test-instance";
