...
import { execute, testEnvironment } from "@yantra/adapter-openclaw-gateway/server";
import {
  buildOpenClawGatewayConfig,
  parseOpenClawGatewayStdoutLine,
} from "@yantra/adapter-openclaw-gateway/ui";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
      ...
      expect(payload?.sessionKey).toBe("yantra:issue:issue-123");
      expect(String(payload?.message ?? "")).toContain("wake now");
      expect(String(payload?.message ?? "")).toContain("YANTRA_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("YANTRA_TASK_ID=task-123");
      expect(String(payload?.message ?? "")).toContain("## Yantra Wake Payload");
      ...
      expect(payload?.yantra).toMatchObject({
        wake: {
          latestCommentId: "comment-2",
          commentIds: ["comment-1", "comment-2"],
        },
