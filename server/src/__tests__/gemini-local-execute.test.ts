...
import { execute } from "@yantra/adapter-gemini-local/server";
...
describe("gemini execute", () => {
  it("passes prompt via --prompt and injects yantra env vars", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "yantra-gemini-execute-"));
    ...
    try {
      ...
      expect(promptArg).toContain("Follow the yantra heartbeat.");
      expect(promptArg).toContain("Yantra runtime note:");
      ...
      expect(invocationPrompt).toContain("Yantra runtime note:");
      expect(invocationPrompt).toContain("YANTRA_API_URL");
      expect(invocationPrompt).toContain("Yantra API access note:");
  ...
  it("always passes --approval-mode yolo", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "yantra-gemini-yolo-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      await execute({
        ...
        config: {
          command: commandPath,
          cwd: workspace,
          env: { YANTRA_TEST_CAPTURE_PATH: capturePath },
        },
        context: {},
        authToken: "t",
        onLog: async () => {},
  ...
  it("uses a compact wake delta instead of the full heartbeat prompt when resuming a session", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "yantra-gemini-resume-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      expect(promptArg).toContain("## Yantra Resume Delta");
      ...
      expect(promptArg).not.toContain("Follow the yantra heartbeat.");
    } finally {
      if (previousHome === undefined) {
        delete process.env.HOME;
      } else {
