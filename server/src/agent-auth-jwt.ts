...
function jwtConfig() {
  const secret = process.env.YANTRA_AGENT_JWT_SECRET;
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.YANTRA_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.YANTRA_AGENT_JWT_ISSUER ?? "yantra",
    audience: process.env.YANTRA_AGENT_JWT_AUDIENCE ?? "yantra-api",
  };
}
