...
function extractUiParserSource(
  ...
  const contractVersion = pkg.yantra?.adapterUiParser;
  if (contractVersion) {
    ...
    logger.info(
      { packageName },
      "Adapter has ./ui-parser export but no yantra.adapterUiParser version — loading anyway (future versions may require it)",
    );
  }
