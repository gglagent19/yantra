const FAVICON_BLOCK_START = "<!-- YANTRA_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- YANTRA_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- YANTRA_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- YANTRA_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.YANTRA_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.YANTRA_WORKTREE_NAME) ?? nonEmpty(env.YANTRA_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.YANTRA_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="yantra-worktree-enabled" content="true" />',
    `<meta name="yantra-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="yantra-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="yantra-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
