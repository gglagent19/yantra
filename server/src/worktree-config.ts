...
import type { YantraConfig } from "@yantra/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by Yantra worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.YANTRA_IN_WORKTREE !== "true") return null;

  const configPath = resolveYantraConfigPath(overrideConfigPath);
  const envPath = resolveYantraEnvPath(configPath);
  const worktreeRoot = path.resolve(path.dirname(configPath), "..");
  const worktreeName = nonEmpty(env.YANTRA_WORKTREE_NAME) ?? path.basename(worktreeRoot);
  const instanceId = nonEmpty(env.YANTRA_INSTANCE_ID) ?? sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(env.YANTRA_HOME) ??
      nonEmpty(env.YANTRA_WORKTREES_DIR) ??
      "~/.yantra-worktrees",
  );
...
function writeConfigFile(configPath: string, config: YantraConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".yantra", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as YantraConfig;
...
): { config: YantraConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.YANTRA_HOME = context.homeDir;
  process.env.YANTRA_INSTANCE_ID = context.instanceId;
  process.env.YANTRA_CONFIG = context.configPath;
  process.env.YANTRA_CONTEXT = context.contextPath;
  process.env.YANTRA_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as YantraConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    YANTRA_HOME: context.homeDir,
    YANTRA_INSTANCE_ID: context.instanceId,
    YANTRA_CONFIG: context.configPath,
    YANTRA_CONTEXT: context.contextPath,
    YANTRA_IN_WORKTREE: "true",
    YANTRA_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: YantraConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as YantraConfig;
  } catch {
    return;
  }
