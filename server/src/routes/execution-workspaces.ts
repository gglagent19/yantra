...
import { issues, projects, projectWorkspaces } from "@yantra/db";
import { updateExecutionWorkspaceSchema } from "@yantra/shared";
...
export function executionWorkspaceRoutes(db: Db) {
  ...
  router.post("/execution-workspaces/:id/runtime-services/:action", async (req, res) => {
    ...
    if (!workspaceCwd) {
      res.status(422).json({ error: "Execution workspace needs a local path before Yantra can manage local runtime services" });
      return;
    }
