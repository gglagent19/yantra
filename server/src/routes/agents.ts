...
import { agents as agentsTable, companies, heartbeatRuns, issues as issuesTable } from "@yantra/db";
...
import { trackAgentCreated } from "@yantra/shared/telemetry";
...
import { runClaudeLogin } from "@yantra/adapter-claude-local/server";
import {
  DEFAULT_CODEX_LOCAL_BYPASS_APPROVALS_AND_SANDBOX,
  DEFAULT_CODEX_LOCAL_MODEL,
} from "@yantra/adapter-codex-local";
import { DEFAULT_CURSOR_LOCAL_MODEL } from "@yantra/adapter-cursor-local";
import { DEFAULT_GEMINI_LOCAL_MODEL } from "@yantra/adapter-gemini-local";
import { ensureOpenCodeModelConfiguredAndAvailable } from "@yantra/adapter-opencode-local/server";
import {
...
export function agentRoutes(db: Db) {
  ...
  const strictSecretsMode = process.env.YANTRA_SECRETS_STRICT_MODE === "true";
  ...
  async function resolveDesiredSkillAssignment(
    ...
    return {
      adapterConfig: writeYantraSkillSyncPreference(adapterConfig, desiredSkills),
      desiredSkills,
      runtimeSkillEntries,
    };
  }
  ...
  router.get("/agents/:id/skills", async (req, res) => {
    ...
    if (!adapter?.listSkills) {
      const preference = readYantraSkillSyncPreference(
  ...
  router.get("/companies/:companyId/live-runs", async (req, res) => {
    ...
    const columns = {
      ...
      activeSkills: sql<Array<{ key: string; name: string; description: string | null }> | null>`${heartbeatRuns.contextSnapshot} -> 'yantraActiveSkills'`.as("activeSkills"),
    };
