...
import type { DeploymentExposure, DeploymentMode } from "@yantra/shared";
import {
  forbidden,
  conflict,
  notFound,
...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "yantra" &&
    normalized !== "yantra-create-agent" &&
    normalized !== "yantra-create-plugin" &&
...
/** Resolve the Yantra repo skills directory (built-in / managed skills). */
function resolveYantraSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const yantraSkillsDir = resolveYantraSkillsDir();

  // Build set of Yantra-managed skill names
  const yantraSkillNames = new Set<string>();
  if (yantraSkillsDir) {
    try {
      for (const entry of fs.readdirSync(yantraSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) yantraSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isYantraManaged: yantraSkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.yantraApiUrl)) {
    const legacyYantraApiUrl = nonEmptyTrimmedString(input.yantraApiUrl);
    if (legacyYantraApiUrl) merged.yantraApiUrl = legacyYantraApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    yantraApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.yantraApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawYantraApiUrl =
    typeof defaults.yantraApiUrl === "string"
      ? defaults.yantraApiUrl.trim()
      : "";
  if (rawYantraApiUrl) {
    try {
      const parsedYantraApiUrl = new URL(rawYantraApiUrl);
      if (
        parsedYantraApiUrl.protocol !== "http:" &&
        parsedYantraApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_yantra_api_url_protocol",
          level: "warn",
          message: `yantraApiUrl must use http:// or https:// (got ${parsedYantraApiUrl.protocol}).`
        });
      } else {
        normalized.yantraApiUrl = parsedYantraApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_yantra_api_url_configured",
          level: "info",
          message: `yantraApiUrl set to ${parsedYantraApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_yantra_api_url_invalid",
        level: "warn",
        message: `Invalid yantraApiUrl: ${rawYantraApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Yantra host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Yantra is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm yantraai allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/yantra";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/yantra-claimed-api-key.json and load YANTRA_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: yantraApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Yantra hostname is reachable and allowed via `pnpm yantraai allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Yantra API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/yantra/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # Yantra OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          yantraApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, Yantra generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "yantraApiUrl": "https://yantra-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in Yantra before key claim is allowed.
    ...
    - ~/.openclaw/workspace/yantra-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/yantra-claimed-api-key.json

    And set the YANTRA_API_KEY and YANTRA_API_URL in your environment variables as specified here:
    ...
    Then set YANTRA_API_KEY and YANTRA_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install Yantra skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your YANTRA_API_URL to the top of your skill and note the path to your YANTRA_API_URL
    ...
    ${
      ...
      "Ensure Yantra is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Yantra base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.yantraApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm yantraai allowed-hostname <host>
      - then restart Yantra and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@yantra.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "yantra", path: "/api/skills/yantra" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "yantra-create-agent",
          path: "/api/skills/yantra-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              yantraApiUrl: req.body.yantraApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.yantraApiUrl &&
          !persistedDefaults.yantraApiUrl
        ) {
          missingPersistedFields.push("yantraApiUrl");
        }
  ...
  router.get("/admin/stats", async (req, res) => {
    await assertInstanceAdmin(req);
    const stats = await access.getAdminStats();
    res.json(stats);
  });
