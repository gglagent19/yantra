...
import { claudeConfigDir, parseClaudeStreamJson } from "@yantra/adapter-claude-local/server";
import { codexHomeDir, parseCodexJsonl } from "@yantra/adapter-codex-local/server";
import { parseOpenCodeJsonl } from "@yantra/adapter-opencode-local/server";
import {
...
const FEEDBACK_SCHEMA_VERSION = "yantra-feedback-envelope-v2";
const FEEDBACK_BUNDLE_VERSION = "yantra-feedback-bundle-v2";
const FEEDBACK_PAYLOAD_VERSION = "yantra-feedback-v1";
const FEEDBACK_DESTINATION = "yantra_labs_feedback_v1";
...
function captureStatusFromFiles(files: FeedbackTraceBundleFile[]): FeedbackTraceBundleCaptureStatus {
  ...
  const hasAdapterFiles = files.some((file) =>
    file.source !== "yantra_run" &&
    file.source !== "yantra_run_events" &&
    file.source !== "yantra_run_log",
  );
...
async function buildOpenCodeTraceFiles(input: {
  ...
  const opencodeRoot = resolveHomeAwarePath(
    process.env.YANTRA_OPENCODE_STORAGE_DIR ?? "~/.local/share/opencode",
  );
...
async function buildAgentContext(
  ...
  const desiredSkillRefs = uniqueNonEmpty(readYantraSkillSyncPreference(adapterConfig).desiredSkills).slice(0, MAX_SKILLS);
...
async function buildFeedbackTraceBundleFromRow(
  ...
  let yantraRun: Record<string, unknown> | null = null;
  ...
  if (!sourceRunId) {
    ...
    if (!run || run.companyId !== row.companyId) {
      ...
      yantraRun = sanitizeFeedbackValue(
        {
          id: run.id,
      ...
      files.push(makeBundleFile({
        path: "yantra/run.json",
        contentType: "application/json",
        source: "yantra_run",
        contents: `${JSON.stringify(yantraRun, null, 2)}\n`,
      }));

      const sanitizedEvents = sanitizeFeedbackValue(
        events,
        state,
        "bundle.yantraRun.events",
        MAX_TRACE_FILE_CHARS,
      );
      files.push(makeBundleFile({
        path: "yantra/run-events.json",
        contentType: "application/json",
        source: "yantra_run_events",
        contents: `${JSON.stringify(sanitizedEvents, null, 2)}\n`,
      }));

      if (logText) {
        files.push(makeBundleFile({
          path: "yantra/run-log.ndjson",
          contentType: "application/x-ndjson",
          source: "yantra_run_log",
          contents: `${sanitizeFeedbackText(logText, state, "bundle.yantraRun.log", MAX_TRACE_FILE_CHARS)}\n`,
        }));
      } else {
