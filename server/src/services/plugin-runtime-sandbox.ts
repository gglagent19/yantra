...
import type { YantraPluginManifestV1 } from "@yantra/shared";
...
export async function loadPluginModuleInSandbox(
  ...
  const loadModuleSync = (modulePath: string): Record<string, unknown> => {
    ...
    const sandboxArgs = {
      __yantra_exports: module.exports,
      __yantra_module: module,
      __yantra_require: requireInSandbox,
      __yantra_filename: realPath,
      __yantra_dirname: path.dirname(realPath),
    };
    // Temporarily inject args into the context, run, then remove to avoid pollution.
    Object.assign(context, sandboxArgs);
    const wrapped = `(function (exports, module, require, __filename, __dirname) {\n${code}\n})(__yantra_exports, __yantra_module, __yantra_require, __yantra_filename, __yantra_dirname)`;
