 ...
 */
import { pluginManifestV1Schema } from "@yantra/shared";
import type { YantraPluginManifestV1 } from "@yantra/shared";
import { PLUGIN_API_VERSION } from "@yantra/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): YantraPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as YantraPluginManifestV1,
        };
      }
