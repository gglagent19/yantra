...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@yantra/db";
 ...
 *
 * This implementation delegates to the core Yantra domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@yantra/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
