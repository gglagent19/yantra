 ...
 *    (`~/.yantra/plugins/`) and `node_modules` for packages matching
 *    the `yantra-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published Yantra plugins.
 * Packages matching this pattern are considered Yantra plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "yantra-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: YantraPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.yantra/plugins/ local directory
  | "npm"               // npm packages matching yantra-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: YantraPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the yantra-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "yantra-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: YantraPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover Yantra plugins installed as npm packages in the current
   * Node.js environment matching the "yantra-plugin-*" naming convention.
   *
   ...
   * the "yantraPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a Yantra plugin.
   * Throws if the package is a Yantra plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<YantraPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: YantraPluginManifestV1;
    newManifest: YantraPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the Yantra plugin naming convention.
 * Accepts both the "yantra-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @yantra/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "yantraPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const yantraPlugin = pkgJson["yantraPlugin"];
  if (
    yantraPlugin !== null &&
    typeof yantraPlugin === "object" &&
    !Array.isArray(yantraPlugin)
  ) {
    const manifestRelPath = (yantraPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: YantraPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumYantraVersion;
}
 ...
 *   packageName: "yantra-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: YantraPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as YantraPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Yantra plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<YantraPluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a Yantra plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasYantraPlugin = "yantraPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasYantraPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<YantraPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: YantraPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @yantra/shared exports). Run those workers through
