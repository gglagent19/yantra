...
import type { BillingType, ExecutionWorkspace, ExecutionWorkspaceConfig } from "@yantra/shared";
import {
  agents,
  agentRuntimeState,
  agentTaskSessions,
...
import { trackAgentFirstHeartbeat } from "@yantra/shared/telemetry";
...
const DEFERRED_WAKE_CONTEXT_KEY = "_yantraWakeContext";
const WAKE_COMMENT_IDS_KEY = "wakeCommentIds";
const YANTRA_WAKE_PAYLOAD_KEY = "yantraWake";
const DETACHED_PROCESS_ERROR_CODE = "process_detached";
const startLocksByAgent = new Map<string, Promise<void>>();
const REPO_ONLY_CWD_SENTINEL = "/__yantra_repo_only__";
...
function enrichWakeContextSnapshot(input: {
  ...
  if (wakeCommentIds.length > 0) {
    ...
    delete contextSnapshot[YANTRA_WAKE_PAYLOAD_KEY];
...
export function mergeCoalescedContextSnapshot(
  ...
  if (mergedCommentIds.length > 0) {
    ...
    delete merged[YANTRA_WAKE_PAYLOAD_KEY];
  }
  return merged;
}

async function buildYantraWakePayload(input: {
  db: Db;
  companyId: string;
  contextSnapshot: Record<string, unknown>;
  issueSummary?:
...
export function heartbeatService(db: Db) {
  ...
  async function executeRun(runId: string) {
    ...
    const issueRef = issueContext
      ...
      : null;
    const yantraWakePayload = await buildYantraWakePayload({
      db,
      companyId: agent.companyId,
    ...
    if (yantraWakePayload) {
      context[YANTRA_WAKE_PAYLOAD_KEY] = yantraWakePayload;
    } else {
      delete context[YANTRA_WAKE_PAYLOAD_KEY];
    }
    ...
    const runtimeConfig = {
      ...resolvedConfig,
      yantraRuntimeSkills: runtimeSkillEntries,
    };
    ...
    if (runtimeServiceIntents.length > 0) {
      context.yantraRuntimeServiceIntents = runtimeServiceIntents;
    } else {
      delete context.yantraRuntimeServiceIntents;
    }
    ...
    if (sessionCompaction.rotate) {
      context.yantraSessionHandoffMarkdown = sessionCompaction.handoffMarkdown;
      context.yantraSessionRotationReason = sessionCompaction.reason;
      context.yantraPreviousSessionId = previousSessionDisplayId ?? runtimeSessionIdForAdapter;
      ...
      delete context.yantraSessionHandoffMarkdown;
      delete context.yantraSessionRotationReason;
      delete context.yantraPreviousSessionId;
    }
    ...
    try {
      ...
      if (runtimeServices.length > 0) {
        context.yantraRuntimeServices = runtimeServices;
        context.yantraRuntimePrimaryUrl =
      ...
      if (issueId && (executionWorkspace.created || runtimeServices.some((service) => !service.reused))) {
        try {
          ...
          await onLog(
            "stderr",
            `[yantra] Failed to post workspace-ready comment: ${err instanceof Error ? err.message : String(err)}\n`,
          );
        }
      }
      ...
      if (adapterManagedRuntimeServices.length > 0) {
        ...
        context.yantraRuntimeServices = combinedRuntimeServices;
        context.yantraRuntimePrimaryUrl =
        ...
        if (issueId) {
          try {
            ...
            await onLog(
              "stderr",
              `[yantra] Failed to post adapter-managed runtime comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
      }
      ...
      if (finalizedRun) {
        ...
        if (issueId && outcome === "succeeded") {
          try {
            ...
            await onLog(
              "stderr",
              `[yantra] Failed to post run summary comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
        await releaseIssueExecutionAndPromote(finalizedRun);
