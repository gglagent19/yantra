#!/usr/bin/env bash
#
# Kill all local Yantra dev server processes (across all worktrees).
#
# Usage:
#   scripts/kill-dev.sh        # kill all yantra dev processes
...
while IFS= read -r line; do
  ...
  node_pids+=("$pid")
  node_lines+=("$line")
done < <(ps aux | grep -E '/yantra(-[^/]+)?/' | grep node | grep -v grep || true)

candidate_pidfiles=()
candidate_pidfiles+=(
  "$HOME"/.yantra/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.yantra/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.yantra/runtime-services/instances/*/db/postmaster.pid
)

for sibling_root in "$REPO_PARENT"/yantra*; do
  [[ -d "$sibling_root" ]] || continue
  candidate_pidfiles+=(
    "$sibling_root"/.yantra/instances/*/db/postmaster.pid
    "$sibling_root"/.yantra/runtime-services/instances/*/db/postmaster.pid
  )
done
...
if [[ ${#node_pids[@]} -eq 0 && ${#pg_pids[@]} -eq 0 ]]; then
  echo "No Yantra dev processes found."
  exit 0
fi

if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Found ${#node_pids[@]} Yantra dev node process(es):"
  echo ""
...
if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Sending SIGTERM to Yantra node processes..."
