...
Environment overrides:
  ...
  PC_DATA=/tmp/data           Yantra data dir to use
  YANTRA_HOST=127.0.0.1    Host passed to the onboarded server
  YANTRA_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/yantra-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/yantra-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export YANTRA_HOST="${YANTRA_HOST:-127.0.0.1}"
export YANTRA_PORT="${YANTRA_PORT:-3100}"
export YANTRA_OPEN_ON_LISTEN="${YANTRA_OPEN_ON_LISTEN:-false}"
...
echo "YANTRA_HOST: $YANTRA_HOST"
echo "YANTRA_PORT: $YANTRA_PORT"
...
env \
  ...
  HOST="$YANTRA_HOST" \
  PORT="$YANTRA_PORT" \
  YANTRA_OPEN_ON_LISTEN="$YANTRA_OPEN_ON_LISTEN" \
  pnpm yantraai onboard --yes --data-dir "$PC_DATA"
