...
const DEFAULT_QUERY = "\"Co-Authored-By: Yantra <noreply@yantra.ing>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/yantra-commit-metrics-cache.json");
...
const YANTRA_EMAIL = "noreply@yantra.ing";
const YANTRA_NAME = "yantra";
...
function printHelp() {
  console.log(`Usage: tsx scripts/yantra-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === YANTRA_EMAIL) || (displayName && displayName.toLowerCase() === YANTRA_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("Yantra commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "yantra-commit-metrics",
