#!/usr/bin/env node
/**
 * Screenshot utility for Yantra UI.
 *
 * Reads the board token from ~/.yantra/auth.json and injects it as a
...
const rawUrl = args[0];
const outPath = args[1] || "/tmp/yantra-screenshot.png";
...
function loadBoardToken() {
  const authPath = path.resolve(os.homedir(), ".yantra/auth.json");
  try {
...
if (!cred) {
  console.error("No board token found in ~/.yantra/auth.json");
  process.exit(1);
}
...
(async () => {
  ...
  try {
    ...
    // Scope the auth header to the Yantra origin only
