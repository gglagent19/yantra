...
COMPANY_ID="${COMPANY_ID:-${YANTRA_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-yantra-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-yantra-openclaw-smoke}"
...
Provide one of:
  YANTRA_AUTH_HEADER=\"Bearer <board-token>\"
  YANTRA_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "Yantra OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.yantra.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
