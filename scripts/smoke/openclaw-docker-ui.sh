...
YANTRA_HOST_PORT="${YANTRA_HOST_PORT:-3100}"
YANTRA_HOST_FROM_CONTAINER="${YANTRA_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.yantra-openclaw.override.yml"
...
detect_yantra_base_url() {
  ...
  for candidate in "$YANTRA_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${YANTRA_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${YANTRA_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
yantra_base_url="$(detect_yantra_base_url || true)"
...
Yantra URL for OpenClaw container:
EOF
  if [[ -n "$yantra_base_url" ]]; then
    cat <<EOF
  $yantra_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${YANTRA_HOST_PORT}
  (Do not use http://127.0.0.1:${YANTRA_HOST_PORT} inside the container.)
  If Yantra rejects the host, run on host machine:
    pnpm yantraai allowed-hostname host.docker.internal
  Then restart Yantra and re-run this script.
EOF
  fi
  cat <<EOF
