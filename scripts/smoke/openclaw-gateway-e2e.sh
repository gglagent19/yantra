...
YANTRA_API_URL="${YANTRA_API_URL:-http://127.0.0.1:3100}"
API_BASE="${YANTRA_API_URL%/}/api"
...
OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-${OPENCLAW_TMP_DIR}/openclaw-yantra-smoke}"
...
YANTRA_API_URL_FOR_OPENCLAW="${YANTRA_API_URL_FOR_OPENCLAW:-http://host.docker.internal:3100}"
...
AUTH_HEADERS=()
if [[ -n "${YANTRA_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=( -H "Authorization: ${YANTRA_AUTH_HEADER}" )
fi
if [[ -n "${YANTRA_COOKIE:-}" ]]; then
  AUTH_HEADERS+=( -H "Cookie: ${YANTRA_COOKIE}" )
  YANTRA_BROWSER_ORIGIN="${YANTRA_BROWSER_ORIGIN:-${YANTRA_API_URL%/}}"
  AUTH_HEADERS+=( -H "Origin: ${YANTRA_BROWSER_ORIGIN}" -H "Referer: ${YANTRA_BROWSER_ORIGIN}/" )
fi
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board auth required. Set YANTRA_COOKIE or YANTRA_AUTH_HEADER."
  fi
...
create_and_approve_gateway_join() {
  ...
  join_payload="$(jq -nc \
    ...
    --arg yantraApiUrl "$YANTRA_API_URL_FOR_OPENCLAW" \
...
persist_claimed_key_artifacts() {
  ...
  local skill_dir="${OPENCLAW_CONFIG_DIR%/}/skills/yantra"
  local claimed_file="${workspace_dir}/yantra-claimed-api-key.json"
  local claimed_raw_file="${workspace_dir}/yantra-claimed-api-key.raw.json"
  ...
  if [[ -n "$container" ]]; then
    ...
    docker cp "$claimed_file" "${container}:/home/node/.openclaw/workspace/yantra-claimed-api-key.json" >/dev/null 2>&1 || true
    docker exec "$container" sh -lc "chmod 644 /home/node/.openclaw/workspace/yantra-claimed-api-key.json" >/dev/null 2>&1 || true
  fi

  if [[ "$AUTO_INSTALL_SKILL" == "1" ]]; then
    api_request "GET" "/skills/yantra"
    assert_status "200"
    {
      echo "YANTRA_API_URL=${YANTRA_API_URL_FOR_OPENCLAW}"
      echo
      printf "%s\n" "$RESPONSE_BODY"
    } > "${skill_dir}/SKILL.md"
...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set YANTRA_API_KEY=${AGENT_API_KEY} in your run context before running Yantra heartbeat steps."
...
main() {
  ...
  wait_http_ready "${YANTRA_API_URL%/}/api/health" 15 || fail "Yantra API health endpoint not reachable"
  api_request "GET" "/health"
  assert_status "200"
  log "yantra health deploymentMode=$(jq -r '.deploymentMode // "unknown"' <<<"$RESPONSE_BODY") exposure=$(jq -r '.deploymentExposure // "unknown"' <<<"$RESPONSE_BODY")"
