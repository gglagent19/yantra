...
const YANTRA_SKILL_ROOT_RELATIVE_CANDIDATES = [
  "../../skills",
  "../../../../../skills",
];

export interface YantraSkillEntry {
  key: string;
  runtimeName: string;
  source: string;
  required?: boolean;
...
interface PersistentSkillSnapshotOptions {
  adapterType: string;
  availableEntries: YantraSkillEntry[];
...
> {
  if (entry.required) {
    return {
      origin: "yantra_required",
      originLabel: "Required by Yantra",
      readOnly: false,
    };
  }
  return {
    origin: "company_managed",
    originLabel: "Managed by Yantra",
    readOnly: false,
  };
}
...
type YantraWakePayload = {
  reason: string | null;
  issue: YantraWakeIssue | null;
  commentIds: string[];
  latestCommentId: string | null;
  comments: YantraWakeComment[];
...
function normalizeYantraWakeIssue(value: unknown): YantraWakeIssue | null {
...
function normalizeYantraWakeComment(value: unknown): YantraWakeComment | null {
...
export function normalizeYantraWakePayload(value: unknown): YantraWakePayload | null {
  const payload = parseObject(value);
  const comments = Array.isArray(payload.comments)
    ? payload.comments
        .map((entry) => normalizeYantraWakeComment(entry))
        .filter((entry): entry is YantraWakeComment => Boolean(entry))
    : [];
  ...
  return {
    ...
    issue: normalizeYantraWakeIssue(payload.issue),
    commentIds,
...
export function stringifyYantraWakePayload(value: unknown): string | null {
  const normalized = normalizeYantraWakePayload(value);
  if (!normalized) return null;
  return JSON.stringify(normalized);
}

export function renderYantraWakePrompt(
  value: unknown,
  ...
  const lines = resumedSession
      ? [
        "## Yantra Resume Delta",
        "",
        "You are resuming an existing Yantra session.",
...
export function buildInvocationEnvForLogs(
  ...
  if (resolvedCommand) {
    merged[options.resolvedCommandEnvKey ?? "YANTRA_RESOLVED_COMMAND"] = resolvedCommand;
  }

  return redactEnvForLogs(merged);
}

export function buildYantraEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const runtimeHost = resolveHostForUrl(
    process.env.YANTRA_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort = process.env.YANTRA_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl = process.env.YANTRA_API_URL ?? `http://${runtimeHost}:${runtimePort}`;
  vars.YANTRA_API_URL = apiUrl;
  return vars;
}
...
export async function resolveYantraSkillsDir(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<string | null> {
  const candidates = [
    ...YANTRA_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listYantraSkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<YantraSkillEntry[]> {
  const root = await resolveYantraSkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
  ...
  try {
    ...
    return entries
      ...
      .map((entry) => ({
        key: `yantraai/yantra/${entry.name}`,
...
export function buildPersistentSkillSnapshot(
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(`Desired skill "${desiredSkill}" is not available from the Yantra skills directory.`);
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      desired: true,
      ...
      sourcePath: null,
      targetPath: null,
      detail: "Yantra cannot find this skill in the local runtime skills directory.",
...
function normalizeConfiguredYantraRuntimeSkills(value: unknown): YantraSkillEntry[] {
  if (!Array.isArray(value)) return [];
  const out: YantraSkillEntry[] = [];
...
export async function readYantraRuntimeSkillEntries(
  config: Record<string, unknown>,
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<YantraSkillEntry[]> {
  const configuredEntries = normalizeConfiguredYantraRuntimeSkills(config.yantraRuntimeSkills);
  if (configuredEntries.length > 0) return configuredEntries;
  return listYantraSkillEntries(moduleDir, additionalCandidates);
}

export async function readYantraSkillMarkdown(
  moduleDir: string,
  skillKey: string,
): Promise<string | null> {
...
export function readYantraSkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
  const raw = config.yantraSkillSync;
...
function canonicalizeDesiredYantraSkillReference(
  reference: string,
...
export function resolveYantraDesiredSkillNames(
  config: Record<string, unknown>,
  availableEntries: Array<{ key: string; runtimeName?: string | null; required?: boolean }>,
): string[] {
  const preference = readYantraSkillSyncPreference(config);
  ...
  const desiredSkills = preference.desiredSkills
    .map((reference) => canonicalizeDesiredYantraSkillReference(reference, availableEntries))
    .filter(Boolean);
  return Array.from(new Set([...requiredSkills, ...desiredSkills]));
}

export function writeYantraSkillSyncPreference(
  config: Record<string, unknown>,
  desiredSkills: string[],
): Record<string, unknown> {
  const next = { ...config };
  const raw = next.yantraSkillSync;
  const current =
  ...
  next.yantraSkillSync = current;
  return next;
}

async function symlinkOrCopy(linkSource: string, linkTarget: string): Promise<void> {
  try {
    await fs.symlink(linkSource, linkTarget);
  } catch {
    // Symlink may fail on Windows without Developer Mode / elevated privileges.
    // Fall back to a recursive copy so the skill files are still available.
    await fs.cp(linkSource, linkTarget, { recursive: true });
  }
}

export async function ensureYantraSkillSymlink(
  source: string,
  target: string,
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    ...
    // These vars leak in when the Yantra server itself is started from
    // within a Claude Code session (e.g. `npx yantraai run` in a terminal
