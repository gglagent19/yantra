...
Example Yantra plugin that demonstrates:
...
```bash
pnpm --filter @yantra/plugin-file-browser-example build
pnpm yantraai plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm yantraai plugin uninstall yantra-file-browser-example --force
```
...
- Optional: use `yantra-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
