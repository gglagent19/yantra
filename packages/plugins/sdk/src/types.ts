/**
 * Core types for the Yantra plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@yantra/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @yantra/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of Yantra object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the Yantra UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit Yantra domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Yantra domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the Yantra secret provider
...
export interface PluginSecretsClient {
   ...
   * in the Yantra secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@yantra/plugin-sdk/ui`
 */
 ...
 * @example
 * ```ts
 * import { definePlugin } from "@yantra/plugin-sdk";
 *
