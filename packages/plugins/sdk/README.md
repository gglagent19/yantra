...
Official TypeScript SDK for Yantra plugin authors.

- **Worker SDK:** `@yantra/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@yantra/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@yantra/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@yantra/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@yantra/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@yantra/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@yantra/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@yantra/plugin-sdk/ui/hooks` | Hooks only |
| `@yantra/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@yantra/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@yantra/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@yantra/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@yantra/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@yantra/plugin-sdk/types` | Worker context and API types (advanced) |
...
- Plugin UI bundles run as same-origin JavaScript inside the main Yantra app. They can call ordinary Yantra HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Yantra instance at runtime.
...
```ts
import { definePlugin, runWorker } from "@yantra/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@yantra/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core Yantra information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@yantra/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@yantra/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@yantra/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@yantra/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@yantra/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@yantra/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@yantra/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@yantra/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@yantra/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@yantra/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@yantra/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@yantra/plugin-sdk/bundlers";
...
```bash
yantra-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@yantra/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__yantra__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__yantra__/events` streams `reload` SSE events on UI build changes
