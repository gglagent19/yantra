...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://yantra:yantra@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "yantra");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://yantra:yantra@127.0.0.1:${preferredPort}/yantra`,
  ...
  if (runningPid) {
    ...
    return {
      connectionString: `postgres://yantra:yantra@127.0.0.1:${port}/yantra`,
  ...
  const adminConnectionString = `postgres://yantra:yantra@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "yantra");

  return {
    connectionString: `postgres://yantra:yantra@127.0.0.1:${selectedPort}/yantra`,
