...
Use when:
- You want Yantra to run Pi (the AI coding agent) locally as the agent runtime
...
- Yantra requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/yantras/ and resumed with --session.
