...
import type { AdapterModel } from "@yantra/adapter-utils";
import { asString, runChildProcess } from "@yantra/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.YANTRA_PI_COMMAND === "string" &&
    process.env.YANTRA_PI_COMMAND.trim().length > 0
      ? process.env.YANTRA_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["YANTRA_", "npm_", "NPM_"] as const;
