...
Use when:
- You want Yantra to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the Yantra server.
...
- yantraApiUrl (string, optional): absolute Yantra base URL advertised in wake text
- claimedApiKeyPath (string, optional): path to the claimed API key JSON file read by the agent at wake time (default ~/.openclaw/workspace/yantra-claimed-api-key.json)

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default yantra)

Standard outbound payload additions:
- yantra (object): standardized Yantra context added to every gateway agent request
- yantra.workspace (object, optional): resolved execution workspace for this run
- yantra.workspaces (array, optional): additional workspace hints Yantra exposed to the run
- yantra.workspaceRuntime (object, optional): reserved workspace runtime metadata when explicitly supplied outside normal heartbeat execution
