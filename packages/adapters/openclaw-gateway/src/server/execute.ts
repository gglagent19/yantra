...
const DEFAULT_CLIENT_VERSION = "yantra";
...
function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "yantra";
  if (input.strategy === "run") return `yantra:run:${input.runId}`;
  if (input.strategy === "issue" && input.issueId) return `yantra:issue:${input.issueId}`;
  return fallback;
}
...
function resolveYantraApiUrlOverride(value: unknown): string | null {
...
const DEFAULT_CLAIMED_API_KEY_PATH = "~/.openclaw/workspace/yantra-claimed-api-key.json";

function resolveClaimedApiKeyPath(value: unknown): string {
  return nonEmpty(value) ?? DEFAULT_CLAIMED_API_KEY_PATH;
}

function buildYantraEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const yantraApiUrlOverride = resolveYantraApiUrlOverride(ctx.config.yantraApiUrl);
  const yantraEnv: Record<string, string> = {
    ...buildYantraEnv(ctx.agent),
    YANTRA_RUN_ID: ctx.runId,
  };

  if (yantraApiUrlOverride) {
    yantraEnv.YANTRA_API_URL = yantraApiUrlOverride;
  }
  if (wakePayload.taskId) yantraEnv.YANTRA_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) yantraEnv.YANTRA_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) yantraEnv.YANTRA_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) yantraEnv.YANTRA_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) yantraEnv.YANTRA_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    yantraEnv.YANTRA_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }
...
function buildWakeText(
  payload: WakePayload,
  yantraEnv: Record<string, string>,
  structuredWakePrompt: string,
): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/yantra-claimed-api-key.json";
  const orderedKeys = [
    "YANTRA_RUN_ID",
    "YANTRA_AGENT_ID",
    "YANTRA_COMPANY_ID",
    ...
    "YANTRA_TASK_ID",
    "YANTRA_WAKE_REASON",
    "YANTRA_WAKE_COMMENT_ID",
    "YANTRA_APPROVAL_ID",
    "YANTRA_APPROVAL_STATUS",
    "YANTRA_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = yantraEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = yantraEnv.YANTRA_API_URL ?? "<set YANTRA_API_URL>";

  const lines = [
    "Yantra wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `YANTRA_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load YANTRA_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $YANTRA_API_KEY on every API call.",
    "- Use X-Yantra-Run-Id: $YANTRA_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: YANTRA_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$YANTRA_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\",\"in_review\"]}",
    ...
    "   - GET /api/companies/$YANTRA_COMPANY_ID/issues?assigneeAgentId=$YANTRA_AGENT_ID&status=todo,in_progress,in_review,blocked",
...
function buildStandardYantraPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  yantraEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateYantra = parseObject(payloadTemplate.yantra);
  const workspace = asRecord(ctx.context.yantraWorkspace);
  const workspaces = Array.isArray(ctx.context.yantraWorkspaces)
    ? ctx.context.yantraWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.yantraRuntimeServiceIntents)
    ? ctx.context.yantraRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardYantra: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: yantraEnv.YANTRA_API_URL ?? null,
  };
  const structuredWake = parseObject(ctx.context.yantraWake);
  if (Object.keys(structuredWake).length > 0) {
    standardYantra.wake = structuredWake;
  }

  if (workspace) {
    standardYantra.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardYantra.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardYantra.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "yantra-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const yantraEnv = buildYantraEnvForWake(ctx, wakePayload);
  const structuredWakePrompt = renderYantraWakePrompt(ctx.context.yantraWake);
  const structuredWakeJson = stringifyYantraWakePayload(ctx.context.yantraWake);
  ...
  const yantraPayload = buildStandardYantraPayload(ctx, wakePayload, yantraEnv, payloadTemplate);
  ...
  agentParams.yantra = yantraPayload;
