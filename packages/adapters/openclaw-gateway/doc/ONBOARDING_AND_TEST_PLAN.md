...
## Scope
This plan is now **gateway-only**. Yantra supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Yantra skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a Yantra task.
...
```bash
pnpm --filter @yantra/server test:run -- openclaw-gateway
pnpm --filter @yantra/server typecheck
pnpm --filter @yantra/ui typecheck
pnpm --filter yantraai typecheck
```
