...
Use when:
- You want Yantra to run Cursor Agent CLI locally as the agent runtime
...
- Yantra auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$yantra" and related skills on local runs.
- Yantra auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
